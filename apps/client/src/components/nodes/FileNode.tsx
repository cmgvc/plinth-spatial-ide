...
export default function FileNode({ id, data }: FileNodeProps) {
  ...
  return (
    <div className={`flex flex-col h-full bg-[#1e1e1e] border border-[#333] rounded-md shadow-2xl overflow-hidden ring-1 ring-white/10 ${isEditing ? "nodrag" : ""}`}>
      ...
      {/* Header */}
      <div className="flex items-center justify-between bg-[#252526] px-3 py-2 border-b border-[#333] drag-handle" style={{ borderTop: `2px solid ${theme.color}` }}>
        <div className="flex items-center gap-3">
          <span className="text-[11px] text-gray-400 font-mono truncate max-w-[180px]">{data.filename}</span>
          
          {!isImage && (
            <>
              <button
                onClick={() => setIsEditing(!isEditing)}
                className={`text-[9px] px-2 py-0.5 rounded border transition-all ${isEditing ? "bg-blue-500/20 border-blue-500 text-blue-300" : "border-[#444] text-gray-400"}`}
              >
                {isEditing ? "EDITOR: ON" : "EDITOR: OFF"}
              </button>
              <button
                onClick={() => setIsScrollLocked(!isScrollLocked)}
                className={`text-[9px] px-2 py-0.5 rounded border transition-all ${isScrollLocked ? "border-red-900/50 text-red-500/70" : "bg-green-500/20 border-green-500 text-green-300"}`}
              >
      ...
      <div className="flex flex-1 overflow-hidden relative bg-[#1e1e1e]">
        {isImage ? (
          /* Image View */
          <div className="flex-1 flex items-center justify-center p-4 bg-[#111] overflow-auto custom-scrollbar">
            {imageUrl ? (
              <img 
                src={imageUrl} 
                alt={data.filename} 
                className="max-w-full max-h-full object-contain shadow-2xl rounded pointer-events-none"
              />
            ) : (
              <div className="text-[9px] uppercase tracking-widest text-gray-700">Loading Image...</div>
            )}
          </div>
        ) : (
          /* Code View */
          <>
            <div ref={gutterRef} className="bg-[#1c1c1c] text-[#6e7681] text-right px-3 py-4 select-none border-r border-[#333] min-w-[45px] font-mono text-[12px] leading-6 overflow-hidden">
            ...
            >
              <div className="grid grid-cols-1 grid-rows-1 min-h-full w-full">
                ...
                />
                <pre className={`col-start-1 row-start-1 p-4 font-mono text-[13px] leading-6 text-[#d4d4d4] whitespace-pre transition-opacity duration-75 ${!isEditing ? "opacity-100" : "opacity-0"}`}>
                  <code>{localCode}</code>
                </pre>
              </div>
            </div>
